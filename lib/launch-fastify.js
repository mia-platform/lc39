...
async function launchFastify(file, options, address) {
  ...
  let openApiSpecification = process.env.OPENAPI_SPECIFICATION || 'openapi'
  openApiSpecification = openApiSpecification.toLowerCase()
  ...
  if (options.exposeMetrics) {
    fastify.register(fp(metricsRoutes), { serviceModule, logLevel: statusLogLevel })
  }
