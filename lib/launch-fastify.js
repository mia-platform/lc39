...
// we support only prefixes like /, /singleword/, /multiple-words-and-numb3rs/
// and also without trailing slash
...
async function launchFastify(serviceModule, /** @type {import('fastify').FastifyHttpOptions} */ options, address) {
  ...
  importEnv(mergedOptions.envPath)
  validatePrefix(mergedOptions.prefix)
  validateLogLevel(mergedOptions.logLevel)

  const logger = customLogger(mergedOptions)
  const fastify = fastifyBuilder({ ...exportFastifyOptions(mergedOptions), logger })

  // Resetting keepAliveTimeout to the options value to avoid that Fastify's serverFactory resets the value
  // to the Node.js default of five seconds.
  fastify.server.keepAliveTimeout = mergedOptions.keepAliveTimeout ?? 72000
  ...
  if (mergedOptions.exposeDocumentation) {
  ...
  if (mergedOptions.exposeMetrics) {
  ...
  fastify
    .after(() => {
      ...
      fastify[kPrefixSymbolKey] = mergedOptions.prefix || ''
    })
  fastify.register(fp(serviceModule), exportServiceOptions(options))

  await fastify.ready()
  if (address) {
    await fastify.listen({ port: mergedOptions.port, host: address })
  }
  return fastify
}
...
module.exports = async function launchFromCli(fileOrModule, options, otelSdk) {
  const fastify = await launchFastifyWithModuleOrFile(fileOrModule, options, '0.0.0.0')
